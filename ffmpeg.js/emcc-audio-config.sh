...
./configure --cross-prefix=em --cc=emcc --enable-cross-compile --target-os=none --arch=x86_32\
 --prefix=$(pwd)/dist --extra-ldflags=-L$(pwd)/dist/lib --extra-cflags=-I$(pwd)/dist/include\
 ...
 --enable-demuxer=wav --enable-decoder=pcm_s16le --enable-decoder=pcm_s24le --enable-decoder=pcm_u8\
 ...
 --enable-demuxer=mov --enable-demuxer=aac --enable-decoder=aac --enable-muxer=mp4\
